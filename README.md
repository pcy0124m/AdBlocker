...
| 手机优化 | 「手机垃圾清理」一键跳转系统「释放空间」，由系统统一清理全机各应用缓存与临时文件；另含本应用自清、各 App 缓存占用排行、后台进程体检 | 无需额外权限（`GET_PACKAGE_SIZE` 普通权限，仅用于读取占用展示） |
...
- **「清理手机垃圾」走系统入口（非直接清）**：免 Root 的普通应用**没有权限直接清除其它 App 的缓存**（`CLEAR_APP_CACHE` 是系统级权限）。因此「手机垃圾清理」按钮实际上是跳转到系统「释放空间 / 存储管理」，由系统统一清理全机缓存——这是普通应用能做的、最彻底也最安全的方式。本应用自己只做「清理自身缓存」（立竿见影）与缓存占用展示。另：不做「一键杀后台」式伪加速，安卓会自动回收内存，强杀反而更耗电、App 重启更慢。
